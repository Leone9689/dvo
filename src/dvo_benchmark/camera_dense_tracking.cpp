...
void CameraDenseTracker::handleConfig(dvo::CameraDenseTrackerConfig& config, uint32_t level)
{
  if(level == 0) return;

  if(level & dvo::CameraDenseTracker_RunDenseTracking)
  {
  ...
  if(level & dvo::CameraDenseTracker_ConfigParam)
  {
  ...
  if(level & dvo::CameraDenseTracker_MiscParam)
  {
    vis_->reset();
    delete vis_;
...
)
{
  ...
  std::cout<<"TRANSFORM:"<<accumulated_transform.matrix()<<std::endl;
